...
/* ---- Slider handle: glossy purple orb ---- */
...
/* ---- Individual scenery per input card (Personal / Eating / Lifestyle) ----
   Marker trick: an invisible div rendered just before each st.container(border=True)
   lets us target that specific card via the CSS adjacent-sibling combinator,
   since Streamlit gives every card the same generic wrapper class. */
.card-marker { display: none; }

.card-marker-personal ~ div[data-testid="stVerticalBlockBorderWrapper"] {
    position: relative;
    overflow: hidden;
    background:
    ...
    border-color: rgba(94, 110, 210, 0.4) !important;
}
.card-marker-personal ~ div[data-testid="stVerticalBlockBorderWrapper"]::after {
    content: "🧍";
    position: absolute;
    right: -18px;
    bottom: -28px;
...
.card-marker-eating ~ div[data-testid="stVerticalBlockBorderWrapper"] {
    position: relative;
    overflow: hidden;
    background:
    ...
    border-color: rgba(224, 130, 70, 0.4) !important;
}
.card-marker-eating ~ div[data-testid="stVerticalBlockBorderWrapper"]::after {
    content: "🍽️";
    position: absolute;
    right: -14px;
    bottom: -28px;
...
.card-marker-lifestyle ~ div[data-testid="stVerticalBlockBorderWrapper"] {
    position: relative;
    overflow: hidden;
    background:
    ...
    border-color: rgba(60, 165, 120, 0.4) !important;
}
.card-marker-lifestyle ~ div[data-testid="stVerticalBlockBorderWrapper"]::after {
    content: "🏃";
    position: absolute;
    right: -14px;
    bottom: -30px;
...
def render_predict_page(model_choice, rf_model, knn_model, svm_model, ann_model,
    ...
    with col1:
        st.markdown('<div class="card-marker card-marker-personal"></div>', unsafe_allow_html=True)
    ...
    with col2:
        st.markdown('<div class="card-marker card-marker-eating"></div>', unsafe_allow_html=True)
        with st.container(border=True):
    ...
    with col3:
        st.markdown('<div class="card-marker card-marker-lifestyle"></div>', unsafe_allow_html=True)
        with st.container(border=True):
