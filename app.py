...
.stApp {
    background:
        radial-gradient(ellipse 90% 55% at 50% 0%, rgba(255,255,255,0.55) 0%, rgba(255,255,255,0) 60%),
        linear-gradient(180deg, #7FB8F0 0%, #A9D4EE 22%, #CDE9DE 45%, #8FD0B8 65%, #4FAE93 100%);
...
/* ---- Slider: metallic chrome track + thumb instead of default red ----
   Streamlit's default accent (red) comes from --primary-color, which several
   internal slider parts reference directly, so we override the variable AND
   add an explicit brushed-metal look on top of the track/thumb ourselves. */
div[data-testid="stSlider"] {
    --primary-color: #8B95A8 !important;
}
/* filled portion of the track: a brushed-steel gradient with a bright
   highlight band, instead of a flat red fill */
div[data-testid="stSlider"] div[data-baseweb="slider"] > div > div:nth-child(2) {
    background: linear-gradient(180deg, #E8ECF2 0%, #B9C2D0 35%, #8B95A8 55%, #6E7893 100%) !important;
    box-shadow: inset 0 1px 0 rgba(255,255,255,0.8), 0 1px 2px rgba(40,25,110,0.2) !important;
}
/* the current-value number above the thumb: metallic text instead of red */
div[data-testid="stSlider"] [data-testid="stTickBarMin"],
div[data-testid="stSlider"] [data-testid="stTickBarMax"] {
    color: #6152C4 !important;
}
...
div[data-testid="stSlider"] > div > div > div[style*="color"] {
    color: #5B6478 !important;
}
div[data-testid="stSlider"] div[role="slider"] {
    background: linear-gradient(155deg, #F4F6FA 0%, #C7CEDA 30%, #939DB0 55%, #6E7893 80%, #565F71 100%) !important;
    border: 2px solid #4A5266 !important;
    box-shadow:
        inset 0 1px 1px rgba(255,255,255,0.9),
        inset 0 -2px 3px rgba(40,45,60,0.35),
        0 3px 6px rgba(40,25,110,0.35) !important;
}
...
/* animated sky/lake backdrop, slow drift instead of static */
.stApp {
...
def render_comparison_charts(df: pd.DataFrame):
    ...
    color_scale = alt.Scale(
        ...
        range=[metallic_gradient(MODEL_COLORS.get(m, "#94A3B8")) for m in models_present],
    )
    ...
    with st.expander("Show all metrics side-by-side (grouped)"):
        ...
        layered = alt.layer(bars, labels).properties(height=280, width=120, background="transparent")
...
def render_predict_page(model_choice, rf_model, knn_model, svm_model, ann_model,
    ...
    if st.session_state.get("show_prediction"):
        ...
        def show_result(model_name, proba):
            ...
            proba_color_scale = alt.Scale(
                domain=categories,
                range=[metallic_gradient(LABEL_COLORS[LABEL_MAP[i]]) for i in range(len(proba))],
            )
            ...
            def build_proba_chart(frame_df):
                ...
                bars = base.mark_bar(
                    ...
                    color=alt.Color("Category:N", scale=proba_color_scale, legend=None),
